...
$sql = "SELECT  p.Name, b.schedule,v.name,   s.serviceName, b.totalAmount,b.status,u.fname,u.lname,b.id  FROM `bookinglist` b INNER JOIN services s on b.	serviceId = s.id INNER JOIN vehicle_list v on v.id=b.vehicletypeId INNER JOIN provider p on p.Id = b.providerId INNER JOIN users u on u.id= b.userId";
